...
async function getTypeScriptDiagnostics(file, root) {
	...
	if (!ts) {
		return [
			'TypeScript package was not found. Install it with: pnpm add -D typescript'
		];
	}
	...
	if (configFile.error) {
		return [
			formatTsDiagnostic(ts, configFile.error, file, root)
		];
	}
	...
	if (parsedConfig.errors.length > 0) {
		return parsedConfig.errors.map(diagnostic => (
			formatTsDiagnostic(ts, diagnostic, file, root)
		));
	}
	...
	const rootNames = fileInProject
		? parsedFileNames
		: [...parsedFileNames, file];
	...
	const sourceFile = program
		.getSourceFiles()
		.find(source => samePath(source.fileName, file));
...
async function getEslintDiagnostics(file, root) {
	...
	try {
		...
		return [
			'ESLint package was not found. Install it with: pnpm add -D eslint'
		];
	}
	...
	try {
		...
		for (const result of results) {
			...
			for (const message of result.messages) {
				...
				diagnostics.push(
					`${rel}:${line}:${column} - ESLint ${severity}${rule}: ${message.message}`
				);
			}
		}
		...
		return [
			`${rel} - ESLint failed: ${error instanceof Error ? error.message : String(error)}`
		];
	}
}
...
const rawFiles = cliFiles.length > 0
	? cliFiles
	: extractFilesFromCodexInput(input);
...
if (sections.length > 0) {
	const checkedFiles = files
		.map(file => `- ${normalizeForOutput(path.relative(root, file))}`)
		.join('\n');
