...
- `pnpm db:run` — Start Postgres database (Docker)
- `pnpm db:init` — Initialize database with SQL
...
3. Do not use pnpm dev or similar commands the server is already running

### End of Task (MANDATORY)

1. **IMMEDIATELY run ESLint**: `pnpm lint`
2. **If ESLint shows ANY errors**: You MUST fix them - this is NON-NEGOTIABLE
3. **Run format check**: `pnpm format:check`
4. **If format check fails**: Run `pnpm format` to fix
