...
const EducationPreview: FC<PropsType> = ({ resumeInfo, isLoading }) => {
  ...
  const renderEducationDates = (education: NonNullable<DocumentType['educations']>[number]) => {
    const startYear = education?.startDate ? new Date(education.startDate).getFullYear() : '';
    const separator = education?.startDate ? ' - ' : '';
    const endValue = education?.currentlyStudying
      ? t('Present')
      : education?.yearsOnly
        ? education?.endDate
          ? new Date(education.endDate).getFullYear()
          : ''
        : formatDateByLocale(education?.endDate ?? undefined);
    const startValue = education?.yearsOnly ? startYear : formatDateByLocale(education?.startDate ?? undefined);
    return (
            ...
            {education?.educationType !== 'course' && (
              <div className={isDoNotShowDates ? 'block' : 'flex items-start justify-between'}>
                ...
                (!education?.startDate && !education?.endDate)
                  ? null
                  : renderEducationDates(education)}
              </div>
            )}
            {education?.educationType === 'course' &&
              !education?.hideDates &&
