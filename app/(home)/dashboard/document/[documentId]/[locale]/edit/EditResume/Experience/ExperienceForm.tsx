...
const ExperienceForm = () => {
  ...
  return (
    <div>
      ...
      <form>
        <div className="my-3 h-auto w-full divide-y rounded-md border px-3 pb-4">
              ...
              <div className="relative mb-5 grid grid-cols-2 gap-3 pt-4">
                {experiences.length > 1 && (
                  <div className="absolute top-4 -left-8 flex flex-col gap-1">
                    <Button
                      ...
                      disabled={index === 0}
                      aria-label={t('Move experience up')}
                    >
                      <MoveUp size={14} />
                    </Button>
                    <Button
                      ...
                      aria-label={t('Move experience down')}
                    >
                      <MoveDown size={14} />
                    </Button>
                  </div>
                ...
                {experiences.length > 1 && item.id && (
                  <Button
                    variant="secondary"
                    type="button"
                    className="absolute -top-3 -right-5 size-5 rounded-full bg-black! text-center text-white dark:bg-gray-600!"
                    size="icon"
                    disabled={isDeleting}
                    onClick={() => removeExperience(item.id)}
                    aria-label={t('Remove experience')}
                  >
                    <X size="13px" />
                  </Button>
                )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="mt-6 ml-2 flex h-full flex-col justify-start gap-2">
                    <div className="flex items-center">
                      ...
                      />
                      <Label htmlFor={`present-checkbox-${index}`} className="cursor-pointer text-xs select-none">
                        {t('Present')}
                      </Label>
                    </div>
                    <div className="flex items-center">
                      <input
                        ...
                        className={direction === 'rtl' ? 'ml-2' : 'mr-1'}
                      />
                      <Label htmlFor={`years-only-checkbox-${index}`} className="cursor-pointer text-xs select-none">
                        {t('Years Only')}
                      </Label>
                    </div>
                  </div>
