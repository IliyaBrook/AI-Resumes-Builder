...
export const document = pgTable(
  "document",
  {
    ...
    updatedAt: timestamp("updated_at", { mode: "string" })
      .defaultNow()
      .notNull(),
    projectsSectionTitle: varchar("projects_section_title", {
      length: 255,
    }).default("Projects"),
    skillsDisplayFormat: varchar("skills_display_format", { length: 32 }),
    personalInfoDisplayFormat: varchar("personal_info_display_format", {
      length: 32,
    }).default("default"),
  },
  (table) => {
    return {
      documentDocumentIdUnique: unique("document_document_id_unique").on(
        table.documentId
      ),
    };
  }
